  ...
  state.getCurrentState(senderID, function(err, state) {
    ...
    if (state === 'connected') {
      peer.getCurrentPeer(senderID, function(err, res) {
        if (err) throw (err);
        var reply = {};
        reply.body = message.body; //copy text
        if (message.attachments.length > 0) {
          console.log(message.attachments[0]);
          if (message.attachments[0].stickerID) {
            reply.sticker = message.attachments[0].stickerID;
          } else if (message.attachments[0].url) {
            reply.attachment = message.attachments[0].url;
          }
  ...
  api.getUserInfo(senderID, function(err, ret) {
    if (!err) {
      var userInfo = {
        ...
        gender: ret[senderID].gender,
      };

      register.setRegister(senderID, userInfo.gender, function(err, result) {
        if (!err) {
