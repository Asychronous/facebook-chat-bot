    ...
    if (result === 'initial') {
      ...
      quota.getCurrentQuota(id, (err, result) => {
        ...
        if (result.quota > 0) {
          ...
          api.sendMessage('We will find someone for you, please wait... :)', threadID);
...
function doNext(event, api) {
  ...
  peer.getCurrentPeer(senderID, function(err, res) {
    ...
    var reciverID = res.peer;

    state.getCurrentState(reciverID, function(err, result) {
      if (err) throw (err);
      ...
      if (result !== 'connected') {
        ...
        quota.getCurrentQuota(reciverID, (err, result) => {
          if (result.quota > 0) {
            state.setWaitingState(reciverID, (err, success) => {
              if (err) throw (err);
              if (success) {
                api.sendMessage('Starting chat', reciverID);
                console.log('Starting chat');
              } else {
                api.sendMessage('Nobody here', reciverID);
              }
            });

          } else {
            state.setInitialState(reciverID, (err, success) => {
              if (err) throw (err);
              if (success) {
                api.sendMessage('Qutoa is running out', reciverID);
        ...
        quota.getCurrentQuota(senderID, (err, result) => {
          if (result.quota > 0) {
            ...
            state.setInitialState(senderID, (err, success) => {
              if (err) throw (err);
              if (success) {
                api.sendMessage('Qutoa is running out', threadID);
                console.log('setInitialState success');
              }
            });
          }
  ...
  api.getUserInfo(senderID, function(err, ret) {
    if (!err) {
      register.setRegister(senderID, ret[senderID].gender, function(err, result) {
        if (!err) {
          api.sendMessage('You have been registered!', threadID);
